...
def eval_video_tracking_davis(
    ...
    frame1_feat = extract_feature(model, frame1).T
    ...
    for cnt in tqdm(range(1, len(frame_list))):
        ...
        frame_tar_avg = paddle.nn.functional.interpolate(
            frame_tar_avg,
        ...
        _, frame_tar_seg = paddle.max(x=frame_tar_avg, axis=0), paddle.argmax(
            x=frame_tar_avg, axis=0
        )
...
def restrict_neighborhood(h, w):
    mask = paddle.zeros(shape=[h, w, h, w])
    for i in range(h):
        for j in range(w):
...
def label_propagation(
    ...
    feat_sources = paddle.stack(x=list_frame_feats)
    feat_tar = paddle.nn.functional.normalize(x=feat_tar, axis=1, p=2)
    feat_sources = paddle.nn.functional.normalize(x=feat_sources, axis=1, p=2)
    feat_tar = feat_tar.unsqueeze(axis=0).repeat(ncontext, 1, 1)
    aff = paddle.exp(x=paddle.bmm(x=feat_tar, y=feat_sources) / 0.1)
    if args.size_mask_neighborhood > 0:
        if mask_neighborhood is None:
            mask_neighborhood = restrict_neighborhood(h, w)
            mask_neighborhood = mask_neighborhood.unsqueeze(axis=0).repeat(
                ncontext, 1, 1
            )
    ...
    perm_0[2] = 1
    perm_0[1] = 2
    aff = x.transpose(perm=perm_0).reshape(-1, h * w)
    tk_val, _ = paddle.topk(k=args.topk, x=aff, axis=0)
    tk_val_min, _ = paddle.min(x=tk_val, axis=0), paddle.argmin(x=tk_val, axis=0)
    aff[aff < tk_val_min] = 0
    aff = aff / paddle.sum(aff, keepdim=True, axis=0)
    list_segs = [s for s in list_segs]
    segs = paddle.concat(x=list_segs)
    nmb_context, C, h, w = segs.shape
    x = segs.reshape(nmb_context, C, -1)
    perm_1 = list(range(x.ndim))
    perm_1[2] = 1
    perm_1[1] = 2
    segs = x.transpose(perm=perm_1).reshape(-1, C).T
    seg_tar = paddle.mm(input=segs, mat2=aff)
...
def extract_feature(model, frame, return_h_w=False):
    ...
    out = model.get_intermediate_layers(frame.unsqueeze(axis=0), n=1)[0]
    out = out[:, 1:, :]
...
def imwrite_indexed(filename, array, color_palette):
    ...
    im.putpalette(color_palette.flatten())
...
def to_one_hot(y_tensor, n_dims=None):
    ...
    _, h, w = y_tensor.shape
    y_tensor = y_tensor.astype(paddle.Tensor).view(-1, 1)
    n_dims = n_dims if n_dims is not None else int(paddle.max(x=y_tensor)) + 1
    y_one_hot = paddle.zeros(shape=[y_tensor.shape[0], n_dims]).put_along_axis_(
        axis=1, indices=y_tensor, values=1
    )
    y_one_hot = y_one_hot.view(h, w, n_dims)
    return y_one_hot.transpose(perm=[2, 0, 1]).unsqueeze(axis=0)
...
def read_seg(seg_dir, factor, scale_size=[480]):
    ...
    small_seg = (
        paddle.to_tensor(data=small_seg.copy()).contiguous().float().unsqueeze(0)
    )
...
def color_normalize(x, mean=[0.485, 0.456, 0.406], std=[0.228, 0.224, 0.225]):
    for t, m, s in zip(x, mean, std):
        t.subtract_(y=paddle.to_tensor(m))
        t.divide_(y=paddle.to_tensor(s))
    return x
