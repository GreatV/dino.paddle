...
if __name__ == "__main__":
    ...
    if os.path.isfile(args.pretrained_weights):
        ...
        if url is not None:
            ...
            state_dict = paddle.hub.load_state_dict_from_url(
    ...
    if args.threshold is not None:
        val, idx = paddle.sort(x=attentions), paddle.argsort(x=attentions)
        val /= paddle.sum(x=val, axis=1, keepdim=True)
        cumval = paddle.cumsum(x=val, axis=1)
        th_attn = cumval > 1 - args.threshold
        idx2 = paddle.argsort(x=idx)
        for head in range(nh):
            th_attn[head] = th_attn[head][idx2[head]]
        th_attn = th_attn.reshape(nh, w_featmap, h_featmap).astype(dtype="float32")
        th_attn = (
            F.interpolate(
                x=th_attn.unsqueeze(axis=0),
    ...
    attentions = (
        F.interpolate(
            x=attentions.unsqueeze(axis=0), scale_factor=args.patch_size, mode="nearest"
        )[0]
        .cpu()
        .numpy()
    )
    os.makedirs(args.output_dir, exist_ok=True)
    paddle.vision.utils.save_image(
        paddle.vision.utils.make_grid(img, normalize=True, scale_each=True),
