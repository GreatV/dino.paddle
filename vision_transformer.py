...
class VisionTransformer(nn.Layer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        out_1 = paddle.create_parameter(
            ...
            default_initializer=nn.initializer.Assign(
                paddle.zeros(shape=[1, num_patches + 1, embed_dim])
            ),
        )
        out_1.stop_gradient = not True
        self.pos_embed = out_1
    ...
    def interpolate_pos_encoding(self, x, w, h):
        ...
        patch_pos_embed = F.interpolate(
            x=patch_pos_embed.reshape((1, int(math.sqrt(N)), int(math.sqrt(N)), dim)).transpose(perm=[0, 3, 1, 2]),
        ...
        patch_pos_embed = patch_pos_embed.transpose(perm=[0, 2, 3, 1]).reshape((1, -1, dim))
    ...
    def __init__(
        ...
        if nlayers == 1:
            ...
            for _ in range(nlayers - 2):
                ...
                layers.append(nn.GELU())
            layers.append(
                nn.Linear(in_features=hidden_dim, out_features=bottleneck_dim)
            )
        ...
        paddle.nn.initializer.Constant(value=1.0)(self.last_layer.weight_g)
        if norm_last_layer:
